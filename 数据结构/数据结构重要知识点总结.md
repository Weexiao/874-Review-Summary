...
**若一个有向图中的顶点不能排成一个拓扑序列，则断定该有向图含有顶点数目大于1的强连通分量**，为什么断定该有向图是强连通图呢？因为只有一个顶点的图也是强连通图，所以不对
...
2. 排序算法的关键字比较次数和原始序列无关的算法是—简单选择排序和折半插入排序

3. 排序算法的关键字比较次数和原始序列有关的算法是—交换类算法(冒泡排序和快速排序)

4. **再次比较直接插入排序和折半插入排序**

	二者最大的区别在于查找插入位置的方式不同。直接插入按顺序查找的方式，而折半插入按折半查找的方式

5. 若只想知道n个关键字组成的序列中第i个关键字之前的部分排序的序列，冒泡排序、简单选择排序、堆排序都可以满足，但堆排序最快
