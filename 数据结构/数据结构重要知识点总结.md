...
平衡二叉树，简称平衡树(AVL)，即**树上任一结点左右子树高度差不超过1**
...
采用基于递归的深度优先算法，从节点$u$出发，递归深度优先遍历图中节点，若访问到节点v，则输出该扫描路径上的节点。为此设置path数组来存放路径上的节点(初始为空)，d表示路径长度(初始为-1)。查找u到v的简单路径过程说明如下：
...
最小生成树的**性质**：假设$G=(V,E)$是一个带权连通无向图， $U$是顶点集$V$的一个非空自己。若$(u.v)$是一条具有最小权值的边，其中$u \in U, v \in V-U$，则必存在一颗包含$(u, v)$的最小生成树
...
\tag{1}
$$
故此**经过n次迭代后$A^{(n-1)}[i][j]$就是顶点$v_i$到$v_j$的最短路径，$A^{(n-1)}$保存了任意顶点之间的最短路径长度**
...
类似于普通的平衡二叉树，不同的是B-树允许每个节点有多个子节点，**B树中所有结点的孩子个数的最大值称为B树的阶，通常用m表示**。一棵m阶B树**或为空树，或为满足如下特性的m叉树**：
...
故新建节点时，直接申请页大小的空间，计算机内存分配按页对齐，这样就实现了一个节点只需要一次I/O
...
- 兄弟够借：若被删除关键字所在节点删除前的关键字个数低于下限，且与此节点右(或左)兄弟节点关键字个数还很宽裕，则需要调整该节点、右(或左)兄弟节点及其双亲结点
	...
	![image-20220911173454175](%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E9%87%8D%E8%A6%81%E7%9F%A5%E8%AF%86%E7%82%B9%E6%80%BB%E7%BB%93.assets/image-20220911173454175.png)

- 兄弟不够借：若被删除关键字所在结点删除前的关键字个数低于下限，且此时与该节点相邻的左右兄弟节点关键字个数均$=\lceil m/2 \rceil-1$，则将关键字删除后与左(或右)兄弟节点及双亲节点中的关键字进行合并

	![image-20220911173725536](%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84%E9%87%8D%E8%A6%81%E7%9F%A5%E8%AF%86%E7%82%B9%E6%80%BB%E7%BB%93.assets/image-20220911173725536.png)
...
- 树型查找
	...
	B+树中，无论查找成功与否，最终都一定查到最下一层；而B树可能停在中间一层
...
| 2路归并排序  |  $O(n \log n)$   |  $O(n \log n)$   |  $O(n \log n)$   | $O(\log n)$ |  稳定  |
